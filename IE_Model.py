...
class IE_Model(nn.Module):
    def __init__(self, device, bilstm_input_size, bilstm_hidden_size, dropout_prob_ner, dropout_prob_rc):
        ...
        self.head_fc = torch.nn.Linear(2*bilstm_hidden_size, head_vector_representation_size, bias=False, device = device)  # Dense Layer aka Fully Connected Layer
        self.tail_fc = torch.nn.Linear(2*bilstm_hidden_size, tail_vector_representation_size, bias=False, device = device)  # Dense Layer aka Fully Connected Layer
    ...
    def forward(self, word_embeddings):
        ...
        all_potential_pairs = []
        all_potential_pairs_biaf_out = torch.tensor([], device=device)
        all_potential_pairs_probabilities = torch.tensor([], device=device)
