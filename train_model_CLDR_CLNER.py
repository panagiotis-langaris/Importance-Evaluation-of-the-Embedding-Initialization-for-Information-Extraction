...
def train_model_CLDR_CLNER(device, lang_model, k_fold, hyperparameters, model, optimizer, scheduler, rc_loss_probab, dataloader_train, dataloader_valid):
    ...
    for epoch in range(hyperparameters['epochs']):
        ...
        if valid_loss_cmb<= valid_loss_min:
            ...
            valid_no_improvement = 0
            print('Validation loss decreased from ({:.4f} --> {:.4f})! Saving model...'.format(valid_loss_min, valid_loss_cmb))
            print('##########################')
            ...
            torch.save(model.state_dict(), SAVED_MODELS_PATH + lang_model + '_fold_' + str(k_fold) + '_best_model.pth')
